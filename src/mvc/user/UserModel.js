...
const UserModel = {
  ...
  todayLoggedUsers: async () => {
    try {
      const query = `
        ...
        WHERE DATE(user_log.trndate) = CURDATE() GROUP BY user_log.userid;
      `;
  ...
  updateUser: async (user, userid) => {
    try {
      const { fullname, phonenumber, address, userroleid, status } = user;
      const query = 'UPDATE user SET fullname = ?, phonenumber = ?, address = ?, userroleid = ?, status = ? WHERE userid = ?';
      const values = [fullname, phonenumber, address, userroleid, status, userid];
